...
function draw() {
  ...
  if(gameState === "start"){
    ...
    textSize(20);
    fill("white")
    text("1) Use 'up' arrow key to move the player up." , width/2-300 , 100);
    text("2) press 's' to shoot sanitier on virus to kill it." , width/2-300 , 150);
    text("3) You will get 3 chances and after a certain time you will get a medikit to increase your life by 1." , width/2-300 , 200);
    text("4) You will get only 10 sanitizer bottles and you will be able to collect another bottles in the game." , width/2-300 , 250);
    text("5) If your score reaches the target score you will win." , width/2-300 , 300);
    text("Press enter to start the game" , width/2-250 , 350);
    ...
    for (var i = 0; i < enemy.length; i++) {
      ...
      if (life === 1) {
        ...
        if (players.isTouching(enemy.get(i))) {
          ...
          text("Press enter to restart the game or press q to quit" , width/2-200 , height/2);
          gameState = "end";
